...
- **Node.js** >= 18
- **ChromaDB** 向量数据库（Docker 部署或本地安装，见下方）
...
**方式二：本地安装（无 Docker 环境）**
```bash
# 安装 Python 和 pip
pip install chromadb

# 启动 ChromaDB 服务
chroma run --path ./chroma_data --port 8574
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/knowledge-bases/:kbId/documents` | 文档列表（含文件路径和版本） |
| POST | `/api/knowledge-bases/:kbId/documents` | 上传 .md 文件（新增或更新） |
| DELETE | `/api/knowledge-bases/:kbId/documents/:docId` | 删除文档（同时删除文件和向量） |
...
**核心改进：现在系统会保存原始 MD 文件到本地磁盘，支持文件更新和重计算。**

- **GET `/api/knowledge-bases/:kbId/documents`** — 按 `indexed_at DESC` 排序返回文档列表，包含 `file_path` 和 `version` 字段
- **POST `/api/knowledge-bases/:kbId/documents`** — multer 内存存储接收 `.md` 文件（最大 10MB，最多 50 个），逐个文件处理：
  
  **新增文件流程：**
  1. 创建文档记录（状态 `indexing`，版本 1）
  2. **保存原始文件到磁盘**：`data/kb_{kbId}/original/{filename}`
  3. 解析 Markdown：≤500KB 使用 marked lexer 提取纯文本，>500KB 使用轻量级正则清洗避免 OOM
  4. 按知识库的分块参数切分（最大 2000 个 chunk）
  5. 每 20 个 chunk 一批调用 Embedding API，存 ChromaDB（带上 `file_id` 元数据），释放内存
  6. 全部完成后更新文档状态为 `done`，记录文件路径，失败则记录错误信息
  
  **更新文件流程（同名文件再次上传）：**
  1. 识别为已有文件，获取当前版本号并递增
  2. **删除旧的本地文件**
  3. **删除 ChromaDB 中的旧向量数据**（通过 `file_id` 过滤删除）
  4. 更新文档状态为 `indexing`
  5. 保存新文件到磁盘
  6. 重新解析、分块、向量化、存储
  7. 更新文档状态为 `updated`
...
用户上传 .md 文件
   ↓
【新增文件】
   → 保存原始文件到 data/kb_{kbId}/original/{filename}
   → Markdown 解析（≤500KB 用 marked lexer，>500KB 用正则清洗）
   → 文本分块（按标题 + 定长，支持重叠，最大 2000 块）
   → 每 20 块一批 → Embedding API 生成向量 → 存 ChromaDB（带 file_id）
   → 更新数据库记录（文件路径、版本号、状态）

【更新文件】（同名文件再次上传）
   → 删除旧本地文件
   → 删除 ChromaDB 中该 file_id 的所有向量
   → 保存新文件到磁盘
   → 重新解析 → 分块 → Embedding → 存 ChromaDB
   → 更新版本号和状态
...
【未来扩展】检索结果可通过 file_id 找到原始文件，展示完整上下文
```
...
| kb_id | TEXT | 所属知识库 ID |
| filename | TEXT | 原始文件名 |
| file_path | TEXT | 本地文件完整路径 |
| version | INTEGER | 文件版本号（每次更新递增） |
| status | TEXT | 状态：pending/indexing/done/error |
...
- **原始文件持久化存储**：
  - 上传的 MD 文件保存到 `data/kb_{kbId}/original/` 目录
  - 数据库记录文件路径和版本号
  - 支持后续查看原文、重新计算、更新替换
...
- **向量数据清理**：
  - 删除文档时同步清理本地文件和 ChromaDB 向量
  - 更新文件时通过 `file_id` 精确删除旧向量
  - 避免数据冗余和不一致

- **可扩展性设计**：
  - 未来可通过 `file_id` 实现原文查看功能
  - 支持根据 chunk 位置截取上下文片段
  - 为高级 RAG 功能（如引用溯源）奠定基础
...
2. **更换向量数据库**：
   - 实现 `VectorStore` 接口（`server/src/vectorstore/base.ts`）
   - 支持 Milvus、Qdrant、Weaviate 等
   - 修改 `document.ts` 中的实例化代码即可
...
1. **ChromaDB 连接失败**
   - 检查 ChromaDB 服务是否启动
   - 确认 `CHROMA_HOST` 和 `CHROMA_PORT` 配置正确
   - 查看防火墙设置
...
3. **Embedding 失败**
   - 确认 Embedding 服务地址和 API Key 正确
   - 检查网络连接
   - 如果是 llamacpp，确认添加了 `--embeddings` 参数
