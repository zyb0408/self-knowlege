    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onSave}
          disabled={saving}
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white rounded-lg transition-colors"
        >
          {saving ? (
...
export default function GlobalSettingsPage() {
  ...
  const handleSaveSystemPrompt = useCallback(async () => {
    ...
    try {
      ...
      setConfig(updated);
      showSuccess('默认 System Prompt 已保存');
    } catch (err) {
      setError((err as Error).message);
    } finally {
      setSavingEmbedding(false);
        ...
        >
          ...
          {config && (
            <p className="text-xs text-gray-400 mt-2">
              当前 Embedding 模型：
              <code className="bg-gray-100 px-1.5 py-0.5 rounded text-indigo-600">{config.embedding_model}</code>
              ，请确保该服务支持 embeddings API
            </p>
          )}
        </ConfigSection>
